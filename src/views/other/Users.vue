  ...
  >
    <CForm @submit.prevent="handleSave">
      ...
      <CModalBody>
        ...
        <CFormSelect label="Role" v-model="role" required>
...
export default {
  ...
  mounted() {
    vendorService.getVendorList({ role: 'all' }).then(
      (result) => {
  ...
  methods: {
    ...
    handleEdit() {
      ...
      formData.append('role', this.role)
      formData.append('status', this.status)
      formData.append('admin_role', 'all')
    ...
    handleDelete() {
      let data = { id: this.vendor_id, email: this.email, role: 'all' }
