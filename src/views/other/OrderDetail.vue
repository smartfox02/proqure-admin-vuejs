<template>
  <CButton
    ...
    @click="handleBack"
  >
    <!-- <CIcon icon="cil-action-undo" height="22" alt="Logo" style="width: 35px"
  /> -->
    Back</CButton
  >
  <CSmartTable
...
export default {
  ...
  mounted() {
    ...
    console.log('id:', id)
    orderService.getOrderDetailList({ id: id }).then(
      (result) => {
        console.log('orderdetail_result: ', result)
  ...
  methods: {
    ...
    handleStatus() {
      ...
      orderService.changeStatusOrderDetail(data).then((result) => {
        let data = result.data
