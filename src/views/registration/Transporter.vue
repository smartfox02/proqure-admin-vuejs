    ...
    <template #details="{ item }">
      <CCollapse :visible="this.details.includes(item.id)">
        <CCardBody>
          ...
          <p class="text-muted">
            Transporter since: {{ getDateTimeFormat(item.created_at) }}
          </p>
          <CLoadingButton
            size="sm"
            color="info"
...
export default {
  ...
  mounted() {
    transporterService.getTransporterList({ role: 3 }).then(
      (result) => {
  ...
  methods: {
    ...
    handleSave() {
      ...
      transporterService.createTransporter(formData).then((result) => {
    ...
    handleEdit() {
      ...
      const formData = new FormData()
      if (this.image !== null) formData.append('image', this.image)
      formData.append('id', this.transporter_id)
      formData.append('company', this.company)
      formData.append('firstname', this.firstname)
      formData.append('lastname', this.lastname)
      formData.append('phone', this.phone)
      ...
      formData.append('status', this.status)
      formData.append('role', 3)
      transporterService.editTransporter(formData).then((result) => {
        let data = result.data
    ...
    handleDelete() {
      let data = { id: this.transporter_id, email: this.email, role: 3 }
      console.log('handleDelete', data)
      transporterService.deleteTransporter(data).then((result) => {
        let data = result.data
        if (data.status == 0) {
          this.items = data.result[1]
    ...
    handleStatus() {
      ...
      transporterService.changeStatusVender(data).then((result) => {
        let data = result.data
        if (data.status == 0) {
          this.items = data.result[1]
